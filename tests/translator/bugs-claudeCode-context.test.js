...
describe("Claude Code CLI context → OpenAI", () => {
  ...
  // claude-to-openai.js — tool_result images move to a follow-up user message
  it("tool_result image block is preserved", () => {
    ...
    const images = out.messages.flatMap((m) => (Array.isArray(m.content) ? m.content : [])).filter((b) => b.type === "image_url");
    expect(images.map((b) => b.image_url.url)).toEqual(["data:image/png;base64,IMG"]);
  });
});
